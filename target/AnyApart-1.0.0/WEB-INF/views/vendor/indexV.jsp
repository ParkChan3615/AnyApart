        ...
        <div class="card mb-4">
            <div class="card-header">
                <i class="fas fa-chart-area mr-1"></i>
                	아파트 목록 
                	 <a href="${cPath}/vender/apartList.do" style="margin-left: 20%;"><i class="fa fa-desktop" aria-hidden="true"></i> 회원관리</a>
           ...
           <table class="table" id="apartTable">
          ...
          <div class="card mb-4">
            <div class="card-header">
                <i class="fas fa-chart-bar mr-1"></i>
               	<label class="apartName" style="color: blue;">모든 아파트</label>  관리사무소 자주사용하는 매뉴 
            </div>
               <div class="card-body">
						<select class="custom-select" name="yy" onchange="monthselect()">
             			<c:forEach items="${yy}" var="year">
             			<option>${year.year }년</option>
             			</c:forEach>
             		</select>
               <canvas id="officeTimeCont" width="400%" height="600%"></canvas></div>
        </div>
    </div>
     <div class="col-xl-3">
        <div class="card mb-4">
      ...
      <div class="col-xl-3">
        ...
        <div class="card mb-4">
            <div class="card-header">
                <i class="fas fa-chart-bar mr-1"></i>
                	관리사무소 문의 글 <a href="${cPath}/vendor/qna/qnaList.do" style="margin-left: 10%;"><i class="fa fa-plus" aria-hidden="true"></i>더보기</a> 
            </div>
            <div class="card-body" id="list">
	...
	<div class="modal-dialog" data-bs-backdrop="static" style="max-width: 100%; width: auto; display: table;">
	  <div class="modal-content">
	      ...
	      <div class="modal-footer">
			...
			<input type="button" value="저장" class="btn btn-primary mr-3" id="saveBtn" onclick="saveUpdate(); return false;"/>
			<input type="button" value="삭제" class="btn btn-primary mr-3" id="delectBtn" onclick="noticedelete()" />
		...
		if(apartList.length>0){
			$(apartList).each(function(idx, apart){
				...
				tr.append(
						...
						$("<td>").text(apart.aptStart),
						$("<td>").text(apart.aptEnd)
				),
				trTags.push(tr);
			});
		}else{
...
//==================================chartJs=============================
var ctxOffice = document.getElementById('officeTimeCont');
var officeTimeCont = new Chart(ctxOffice, {
	type: 'horizontalBar',//[https://www.chartjs.org/docs/latest/axes/ <<(컨트롤 +우클릭)
							//여기 가서 원하는 타입 바꿔주면 모양바껴용
	data: {
		labels: ["입주민 관리", "차량 관리", '일정 관리','공사/수선관리','회계관리','자산 관리','계약 관리',
			'검침 관리','부과 관리','수납 관리','인사/근태 관리','급여/정산 관리','사이트 관리'],
		datasets: [{
			label: '전월비교',
			data: [412, 555, 321,432, 413, 575, 132,332,212,855, 321,432,200, 0],//값들 입력
			backgroundColor: [
