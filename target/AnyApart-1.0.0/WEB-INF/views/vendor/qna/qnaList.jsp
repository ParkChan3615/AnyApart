...
<div id="container">
	<div class="inner">
		<div class="board-list">
			<br>
			<h4><strong>아파트관리사무소 문의글</strong></h4>
			<br>
			<table>
				<colgroup>
							...
							<tr>
								...
								<td>
									<c:if test="${1 eq board.boDepth and empty board.answerFlag}">
										...
										<input class="btn btn-green" type="button" value="답글하기" onclick="location.href='${insertURL}'">
									</c:if>
									<c:if test="${2 eq board.boDepth }">
										<c:url value="/vendor/qna/qnaUpdate.do" var="updateURL">
											<c:param name="boNo" value="${board.boNo }" />
										</c:url>
										<input class="btn btn-blue" type="button" value="수정하기" onclick="location.href='${updateURL}'">
									</c:if>
									<c:url value="/vendor/qna/qnaDelete.do" var="deleteURL">
										<c:param name="boNo" value="${board.boNo }" />
									</c:url>
									</br>
									<input class="btn btn-red" type="button" value="삭제하기" onclick="location.href='${deleteURL}'">
								</td>
							</tr>
						</c:forEach>
					</c:if>
		...
		<div>
			<form id="searchForm">
				<input type="hidden" name="page" /> <input type="hidden"
					name="boType" /> <input type="hidden" name="searchType"
					value="${pagingVO.searchVO.searchType }" /> <input type="hidden"
