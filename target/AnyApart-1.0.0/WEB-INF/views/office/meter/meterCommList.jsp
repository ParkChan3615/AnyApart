		...
		<c:if test="${fn:length(pagingVO.dataList) != 0}">
			<c:forEach items="${pagingVO.dataList }" var="cmVO">
				<tr>
					...
					<td>${cmVO.commHeat }</td>
					<td>${cmVO.commHotwater }</td>
					<td>${cmVO.commWater }</td>
					<td>${cmVO.commElec }</td>
					<td>
					...
					<c:choose>
						<c:when test="${sysYear-cmVO.commYear gt 1 or (sysYear eq cmVO.commYear and sysMonth-cmVO.commMonth <= 0) or (sysMonth-cmVO.commMonth eq 1 and sysDay<5) }">
							<button type="button" id="updateBtn" class="btn btn-warning" data-commno="${cmVO.commNo }">수정</button>
							<button type="button" id="deleteBtn" class="btn btn-danger" data-commno="${cmVO.commNo }">삭제</button>
						</c:when>
						<c:otherwise>
							부과됨
						</c:otherwise>
