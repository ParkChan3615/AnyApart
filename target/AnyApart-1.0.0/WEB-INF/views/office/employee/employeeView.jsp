...
<div class="d-flex justify-content-end mr-3 mt-3">
	...
	<%-- 		<input type="hidden" name="memId" value="${pagingVO.searchDetail.memId }"/> --%>
			...
			<input type="submit" value="목록" class="btn btn-warning">
		</form>
	</c:if> 
  ...
  <p class="mt-3" id="heading">${employee.empName }님의 정보</p>
  <div id="d-flex justify-content-center" class="mb-4">
	  <span class="title mb-4">사용자코드 : ${employee.memId } &nbsp;&nbsp;/&nbsp;&nbsp;아파트코드 : ${employee.aptCode }</span>
  </div>
...
<%-- 			<img style="width: 200px; height: 200px;" src="#{appInfo['boardFiles']}/${employee.alImg}" /> --%>
		</c:if>
 	...
 	<c:if test="${empty employee.empImg }">
		<p id="noImgNotice">프로필 사진이 없습니다. 사진을 등록해주세요.</p>
	</c:if>
	<table class="table table-borderless ">
		...
		<tr>
			...
			<c:if test="${empty employee.licenseList }">
		 	  <td>
			 	   <p class="text-center">등록된 자격증 정보 없음</p>
		 	  </td>
		 	</c:if>
			...
			<td class="form-inline">
				...
				<c:forEach items="${employee.licenseList }" var="licemployee" varStatus="vs">
					...
					<span id="noticeSpan">* 자격증명을 클릭하면 사본을 확인할 수 있습니다.</span>
			</td>
			</c:if>
		</tr>
		<tr>
...
<div class="modal fade" id="retireModal" tabindex="-1" role="dialog" aria-labelledby="retireModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p>퇴사일 입력</p>
        <input type="date" name="empEndInput" id="empEndInput">
      </div>
      </form>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">닫기</button>
...
<script type="text/javascript">
	...
	$(function(){
		...
		const licenseImageURL = "licenseImage.do?memId=%I&licCode=%C";
		...
		$(".viewImage").on("click", function(){
			...
			modalTag.find(".modal-body").html(
				$("<img>").attr({
						src:licenseImageURL.replace("%I", memId).replace("%C",licCode)
