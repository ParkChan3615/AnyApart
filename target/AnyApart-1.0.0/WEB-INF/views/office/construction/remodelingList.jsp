			...
			success:function(resp){
				...
				if(rmdlList.length>0){
					$(rmdlList).each(function(idx, rmdl){
						if('Y'==rmdl.rmdlYn){
							rmdlYn = $("<button id='cancleBtn' data-rmdlno='"+rmdl.rmdlNo+"'>").text("승인취소")
																.addClass("btn btn-danger");
						}else if('D'==rmdl.rmdlYn){
							rmdlYn = $("<span>").text("취소됨").addClass("badge badge-default");
						}else{
							rmdlYn = $("<button type='button' data-rmdlno='"+rmdl.rmdlNo+"' class='btn btn-primary approvalBtn'>승인하기</button>");
						}
						trTags.push(
							$("<tr>").append(
								$("<td>").text(rmdl.rmdlNo),
